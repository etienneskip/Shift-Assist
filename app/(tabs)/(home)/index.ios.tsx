...
export default function HomeScreen() {
  ...
  const checkUserRoles = async () => {
    ...
    try {
      ...
      console.log('[Home] User roles:', roles);
      setUserRoles(roles);
      ...
      if (roles.includes('support_worker')) {
        console.log('[Home] User is a support worker, redirecting to calendar');
        router.replace('/(tabs)/calendar');
        return;
      }

      // No roles found, show role selection
      console.log('[Home] No roles found, showing role selection');
      setChecking(false);
    } catch (error) {
